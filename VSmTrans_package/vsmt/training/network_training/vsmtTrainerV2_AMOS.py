...
from vsmt.network_architecture.VSmTrans import VSmixTUnet


class vsmtTrainerV2_AMOS(nnUNetTrainer):
    """
    ...
    def initialize_network(self):
        ...
        self.network.num_classes = 16

        if torch.cuda.is_available():
            self.network.cuda()

        import thop
        data = torch.randn((1, 1, 96, 96, 96), dtype=torch.float32).cuda()
        flops, params = thop.profile(self.network, inputs=(data,))
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.initial_lr, weight_decay=self.weight_decay, eps=1e-4)
