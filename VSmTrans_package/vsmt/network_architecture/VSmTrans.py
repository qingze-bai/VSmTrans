...
class VSmixTUnet(SegmentationNetwork):
# class VSmixTUnet(nn.Module):
    def __init__(
        ...
        self.cswinViT = VariableShapeMixedTransformer(
            img_size=img_size,
    ...
    def forward(self, x):
        hidden_states_out = self.cswinViT(x)
...
if __name__ == '__main__':
    ...
    # checkpoint = torch.load(r"../model/model_bestValRMSE.pt", map_location="cpu")
    ...
    # model.cswinViT.load_state_dict(checkpoint["state_dict"]['cswinViT'])
    # out = model(data)
    import thop
